...
const FarmList = () => {
  ...
  const [priceRequestDTO, setPriceRequestDTO] =
    useRecoilState(priceRequestDTOAtom);
  ...
  const handleSearch = async (e) => {
    ...
    if (!searchTerm.trim()) {
      alert("검색어를 입력하세요.");
      setPriceDataState([]); // 검색어가 없을 때는 가격 데이터 초기화
      return;
    }
  ...
  const getAllPrice = async () => {
    setLoading(true); // 데이터 로딩 시작 시 loading 상태 true
    setError(null);
    ...
    try {
      ...
      let transformedData = responses.flatMap((response) => {
        return response.data
          ?.map((item) => {
            ...
            return {
              date:
                item.yyyy && item.regday
                  ? `${item.yyyy}-${item.regday.split("/").join("-")}`
                  : "",
      ...
      if (transformedData.length === 0) {
        if (retryCount.current < 7) {
          ...
          setError("해당 품목을 찾을 수 없습니다.");
          setPriceDataState([]); // 데이터가 없을 경우 가격 데이터 초기화
          return;
        }
      }
      ...
      setPriceDataState([]); // 에러 발생 시 가격 데이터 초기화
      setError("서버 요청 중 오류가 발생했습니다.");
    } finally {
      setLoading(false); // 로딩 종료 후 loading 상태 false
    }
  };
  ...
  return (
    <div className="farmListPage">
      <div className="farmContainer">
        ...
        <div
          className={`farmListContainer ${searchCompleted ? "visible" : ""}`}
        >
                ...
                <>
                  {Object.entries(
                    priceData
                      .filter(
                        (item) =>
                          item.marketname !== "null" && !isNaN(item.price)
                      )
                      .reduce((acc, item) => {
                    ...
                    return (
                      <div key={marketName} className="marketGroupedItem">
                        ...
                        <div>
                          ...
                          <p>
                            가격 :{" "}
                            {new Intl.NumberFormat().format(latestItem.price)}원
                          </p>
