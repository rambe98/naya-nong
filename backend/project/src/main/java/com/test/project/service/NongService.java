...
public class NongService {
	...
	public void verifyPassword(int clientNum, String userPwd) {
		Optional<NongEntity> userEntity 
		= repository.findByClientNumAndUserPwd(clientNum, userPwd);
	...
	public String findUserIdByEmail(String userEmail) {
	    ...
	    if (user.isPresent()) {
	        ...
	        // 이메일 발송
	        emailService.sendEmail(userEmail, "아이디 찾기 결과", 
	        		"당신의 아이디는 " + userId + " 입니다.");
	...
	public String findPasswordByUserIdAndEmail(String userId, String userEmail) {
	    ...
	    if (user.isPresent()) {
	        ...
	        emailService.sendEmail(userEmail, "(나야, 농) 임시 비밀번호", 
	        		"임시 비밀번호는 " + verificationCode + " 입니다. "
	        				+ "로그인 후 보안을 위해 회원 정보 수정을 부탁드립니다.");
